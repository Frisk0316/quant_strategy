...
def _parse_registry_rows(registry_text: str) -> list[_RegistryRow]:
    ...
    for line in registry_text.splitlines():
        ...
        if "planned" in lower_row or "pending" in lower_row:
            continue
        rows.append(_RegistryRow(family_id=cells[3], trials=trials, artifact=artifact, outcome=outcome))
    return rows


def family_registry_from_text(registry_text: str) -> dict[str, FamilyRegistryEntry]:
    families: dict[str, FamilyRegistryEntry] = {}
    for row in _parse_registry_rows(registry_text):
        current = families.get(row.family_id)
        if current is None or row.trials >= current.cumulative_n_trials:
            families[row.family_id] = FamilyRegistryEntry(
                family_id=row.family_id,
                cumulative_n_trials=row.trials,
                status=row.outcome,
...
def _expected_trials(rows: list[_RegistryRow], family_id: str, summary_path: str | Path | None) -> int | None:
    ...
    if summary_path is not None:
        ...
        if exact:
            return exact[-1].trials
    return max(row.trials for row in family_rows)
