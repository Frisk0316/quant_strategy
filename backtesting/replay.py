...
def load_l1_books(
    ...
    candle_inst = fallback_inst_id or inst_id
    candle_path = Path(data_dir) / candle_inst.replace("-", "_") / f"candles_{bar}.parquet"
...
class ReplayBacktestEngine:
    ...
    def _default_instrument_specs(self) -> dict:
        specs = {}
        for symbol in self._cfg.system.symbols:
            specs[symbol] = {"ctVal": 0.01, "minSz": 1, "lotSz": 1, "tickSz": 0.1, "tdMode": "cross"}
        for symbol in self._cfg.system.spot_symbols:
    ...
    async def run(self, feed: HistoricalEventFeed) -> ReplayBacktestResult:
        ...
        order_manager = OrderManager(SimBroker(slippage_bps=2.0), RateLimiter())
        ...
        async def on_order_event(event: Event) -> None:
            recorder.record_order(event.payload, int(event.ts * 1000))
...
def build_feed_for_strategies(
    ...
    market_df = pd.concat([df for df in market_frames if not df.empty], ignore_index=True) if market_frames else pd.DataFrame()
    funding_df = pd.concat([df for df in funding_frames if not df.empty], ignore_index=True) if funding_frames else pd.DataFrame()
