...
def test_minimal_candles_produce_orders(minimal_cfg, tmp_path: Path):
    minimal_cfg.strategies.ma_crossover.fast_window = 2
    minimal_cfg.strategies.ma_crossover.slow_window = 3
...
def test_risk_guard_blocks_are_visible_in_replay_result(minimal_cfg, tmp_path: Path):
    cfg = minimal_cfg.model_copy(deep=True)
    cfg.risk.max_pos_pct_equity = 0.01
    ...
    assert result.rejected_log[0]["reason"] == "position_limit"
