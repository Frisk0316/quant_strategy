...
| H-003 | F-S7-BASIS-MEANREV | 0 | A delta-neutral BTC/ETH perp-vs-spot basis mean-reversion book (enter on extreme basis z-score, exit on convergence) earns a positive net-of-cost Sharpe that beats the static funding-carry baseline, surviving WF and CPCV with DSR >= 0.95 and PSR >= 0.95. | `research/strategy_synthesis.md` Strategy 7; `docs/superpowers/specs/2026-06-25-s7-basis-meanrev-hypothesis.md` | proposed | E-006 (planned) | Pipeline batch 1 candidate S7 (Stage 1). Stage 2 distinctness vs funding_carry: PASS (opposite basis-z regime, bidirectional, convergence not carry). Data-availability GATING: spot canonical candles (BTC-USDT/ETH-USDT) unconfirmed in DB; Codex to verify/load before Stage 3. New family, prior_family_n_trials=0. |
| H-004 | F-S5-RESIDUAL-MEANREV | 0 | A market-neutral basket that removes BTC/ETH common beta and trades residual mean reversion earns a positive net-of-cost Sharpe beating an equal-weight universe basket, surviving WF/CPCV with DSR >= 0.95 and PSR >= 0.95. | `research/strategy_synthesis.md` Strategy 5; `docs/superpowers/specs/2026-06-25-s5-residual-meanrev-hypothesis.md` | proposed | E-007 (planned) | Pipeline batch 1 candidate S5. Stage 2: data PASS (reuses perp universe); distinctness CONDITIONAL - new family only as residual MEAN-REVERSION (residual momentum would be a retry of F-XS-MOMENTUM); HIGHEST overfit risk in batch (in rejected-for-now list). New family, prior=0. |
| H-005 | F-S6-TS-MOMENTUM | 0 | A low-turnover time-series momentum sleeve on BTC/ETH perps (own-trend, vol-targeted, crash-filtered) earns a positive net-of-cost Sharpe beating BTC buy-and-hold, surviving WF/CPCV with DSR >= 0.95 and PSR >= 0.95. | `research/strategy_synthesis.md` Strategy 6; `docs/superpowers/specs/2026-06-25-s6-ts-momentum-hypothesis.md` | proposed | E-008 (planned) | Pipeline batch 1 candidate S6. Stage 2: data PASS; distinctness PASS (TS != CS momentum) with portfolio caveat (returns may correlate with ohlcv_rotation in trends); LOW overfit risk, edge-decay is the honest concern. New family, prior=0. |
