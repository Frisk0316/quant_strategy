...
| H-009 | F-FUNDING-XS-DISPERSION | 0 | A dollar-neutral cross-sectional book that goes long the lowest-trailing-funding-APR quartile and short the highest-trailing-funding-APR quartile of a point-in-time liquid USDT-perp universe, rebalanced weekly, earns a positive net-of-cost Sharpe beating an equal-weight universe basket, surviving WF/CPCV with DSR >= 0.95 and PSR >= 0.95. | `results/idea_batch_20260701_taxonomy_002/idea_batch.json`; `docs/superpowers/specs/2026-06-30-mechanism-taxonomy.md`; `docs/superpowers/specs/2026-07-04-f-funding-xs-dispersion-hypothesis.md` | proposed | E-028 (data-blocked); E-030 (data-available) | E-028's data-availability FAIL was a universe-membership artifact (thin local-parquet mirror), not a real funding-coverage gap; after `scripts/build_universe_membership.py` gained a DB-sourced path and the Stage-2 breadth-min excluded the mechanically-unpassable warmup edge (both 2026-07-03/04, user-approved), E-030 re-probed the same funding source/window/thresholds and data_availability now PASSES (32 eligible symbols, 28 good, breadth min 24 / median 27 vs threshold 10). `stage2_status` stays FAIL pending `distinctness` (vs F-FUNDING-CARRY, see the 2026-07-04 spec's mechanism-level argument) and `cost_after_edge`, which are Stage-2(b)/checkpoint (#9) work, not yet run. Still `proposed` -- no WF/CPCV has been attempted. |
