...
15. **CME gap-fill regime-fitted long_only default** (P1, Route B applied in commit `655af31` 2026-05-19): `allow_direction` default has been flipped from `both` to `long_only` because up-gaps (short BTC) were -3,799 bps vs down-gaps +803 bps on the post-fix yfinance proxy run. Default re-run `results/cme_gap_research_long_only_default.json` reports total_return +7.09%, Sharpe +0.35, MDD -14.7% over 46 trades. **Route B is still explicitly regime-fitted to BTC 2024-26; bear-regime walk-forward remains required before any promotion claim.** If `long_only` fails on a bear or sideways segment in the walk-forward, the default must revert to "do not deploy" and the strategy retired. Even on `long_only`, edge is only ~3 bps above round-trip cost — not validated alpha.
