...
- **Path(s):** `src/okx_quant/execution/deribit_shadow/`,
  `scripts/run_h014_shadow.py`, `src/okx_quant/api/routes_research.py`,
  `frontend/view-research.js`, `config/h014_shadow.yaml`.
- **Responsibility:** One manual or local-UI credential-free daily cycle: reproduce the
...
- **Common failure modes:** F26 (early publication), F39 (stale/wrong day boundary),
  F48 (concurrent UI/scheduler journal append).
...
- **Gotchas:** The shadow price day ends at 08:00 UTC, while the last hourly
  ...
  CLI, UI, and scheduled cycles share `journal.jsonl.lock`; an overlap fails
  closed instead of waiting or writing duplicate evidence.
