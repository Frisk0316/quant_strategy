...
- **覆蓋率是人工審核步驟（2026-07-12 起明文化）**：`check_human_overview.py`
  只驗格式，不驗覆蓋。session 結束更新 `review_index.md` 時，人（或 review
  session）必須確認：本次每個新增/大改的 planning、governance 批次都有一份
  `current` 的 overview 對應；已完成或被取代的工作，其 overview 要標
  `deprecated` 並填 `superseded_by`。不足之處記入 `docs/KNOWN_ISSUES.md`。
