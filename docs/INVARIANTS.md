...
| I39 | Coin-denominated options PnL settles on official Deribit delivery prices, allows bounded-coin-loss structures only, and every daily mark records its source. ADR-0011 additionally rejects naked short-put intents before fill handling, caps each symbol at 1.0 open unit, journals chain failures/rejections without aborting the sibling currency cycle, and admits only one cross-process journal-producing cycle at a time. | R8.1–R8.7 | `tests/unit/test_h014_options_accounting.py`, `tests/unit/test_h014_shadow.py` |
