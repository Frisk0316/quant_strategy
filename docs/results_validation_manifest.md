...
## 2026-06-24 DSR Recheck

`python scripts/recheck_dsr.py` scanned current `results/**/*.json` after the
DSR harness fix commit `fecdd98`. It found 45 JSON rows with DSR fields:
7 CPCV rows and 38 replay-level single-run diagnostic rows. The single-run rows
set `dsr == psr`; they are not CPCV multiple-trial DSR and are not affected by
the old CPCV DSR inflation bug.

| Path | Old DSR | New DSR | PSR | Audit status | Citation rule |
|---|---:|---:|---:|---|---|
| `results/ui_daily_winner_45bdb32b/result.json` | 0.0 | 6.81623e-39 | 0.658074 | recomputed from saved returns | Validation-only Daily Winner; not promotion evidence. |
| `results/xs_momentum_validation_20260623/cpcv.json` | 1.0 | n/a | 0.992355 | untrusted (`DSR > PSR`) | Invalid leaked run; do not cite. |
| `results/xs_momentum_validation_20260623/summary.json` | 1.0 | n/a | 0.992355 | untrusted (`DSR > PSR`) | Invalid leaked run; do not cite. |
| `results/xs_momentum_validation_20260624_leakfix/cpcv.json` | 1.0 | n/a | 0.796052 | untrusted (`DSR > PSR`) | Leak-free but pre-DSR-fix DSR is untrusted; PSR already fails gate. |
| `results/xs_momentum_validation_20260624_leakfix/summary.json` | 1.0 | n/a | 0.796052 | untrusted (`DSR > PSR`) | Leak-free but pre-DSR-fix DSR is untrusted; PSR already fails gate. |
| `results/xs_momentum_validation_20260624_portfoliovol/cpcv.json` | 0.782298 | n/a | 0.823431 | summary-only invariant pass | Raw path returns were not saved; DSR/PSR both fail gate, so no promotion support. |
| `results/xs_momentum_validation_20260624_portfoliovol/summary.json` | 0.782298 | n/a | 0.823431 | summary-only invariant pass | Raw path returns were not saved; DSR/PSR both fail gate, so no promotion support. |
