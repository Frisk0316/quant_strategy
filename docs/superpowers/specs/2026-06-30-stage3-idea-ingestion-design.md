...
- **2026-07-01 展開為可交付設計 + Codex 任務**:見
  [2026-07-01-pipeline-orchestration-driver-design.md](2026-07-01-pipeline-orchestration-driver-design.md)。
  關鍵誠實限制:Stage 2 探測與 Stage 3 回測目前都是逐 family 手刻,不是通用引擎,所以
  driver v1 只自動化「已有程式碼的 family 一路跑到短名單」,沒有程式碼的 family 誠實停在
  `awaiting_stage2_implementation` / `awaiting_stage3_implementation`,不是全自動生程式碼。
