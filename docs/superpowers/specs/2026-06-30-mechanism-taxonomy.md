...
| F-XVENUE-LEADLAG | 跨所價格領先落後(Binance↔OKX) | 資訊傳遞延遲 | **partial-available**(多所 candles) | frontier-unvetted | F-XS-MOMENTUM | 中(容量受限) |
...
- **資料上目前真正可行的 frontier**:`F-FUNDING-XS-DISPERSION`(funding 齊)、
  `F-XVENUE-LEADLAG`(多所 candles),以及掛在 base family 上的 `F-VOL-REGIME` overlay。
  發想優先往這幾格,先驗品質較高且 Stage-2 不會立刻 data-block。
- **目前 data-blocked(Stage 2 會直接 skip)**:OI、liquidation、on-chain、options/IV、
  完整 L2/trade tape。要解鎖這些 family,得先做資料 ingestion(屬另一條工作流,非本管線)。
