...
const { useState: useTradesState, useEffect: useTradesEffect, useMemo: useTradesMemo } = React;
      ...
      <div className="card">
        <div className="card-h">
          <div>
            ...
            <div className="card-sub">{selectedRunId ? "selected run trades.csv" : "MOCK fallback data"}</div>
          </div>
          ...
          <table className="tbl">
            <thead>
              <tr>
                ...
                <th className="num">Price</th><th className="num">Qty</th><th className="num">Notional</th>
                <th className="num">Fee</th><th className="num">PnL</th><th>Status</th><th>Strategy</th>
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, 80).map((t, i) => (
                <tr key={`${t.id}-${i}`}>
                  ...
                  <td><span className={`chip ${t.status === "FILLED" ? "profit" : t.status === "REJECTED" ? "loss" : "warn"}`} style={{ fontSize: 10 }}>{t.status}</span></td>
                  <td className="mono" style={{ color: "var(--text-subtle)", fontSize: 11 }}>{t.strategy}</td>
                </tr>
              ))}
              {!filtered.length && <tr><td colSpan={12} className="field-hint" style={{ textAlign: "center", padding: 16 }}>No trades match the current filters.</td></tr>}
            </tbody>
          </table>
        </div>
  ...
  return (
    <div className="col" style={{ gap: "var(--gap-lg)" }}>
      <div className="card">
        <div className="card-h">
          <div>
            <div className="card-title">Run selector</div>
            <div className="card-sub">choose saved runs to align on a shared timeline</div>
          </div>
        </div>
        <div className="row wrap" style={{ gap: 8 }}>
          {allRuns.map((r) => (
            <button key={r.run_id} className={`btn sm ${selectedIds.includes(r.run_id) ? "" : "ghost"}`} onClick={() => toggleRun(r.run_id)}>
              <span className="mono">{r.run_id.slice(-8)}</span>
              <span style={{ marginLeft: 6 }}>{fmt.pct(r.total_return)}</span>
            </button>
          ))}
          {!allRuns.length && <div className="field-hint">No saved runs found.</div>}
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-h">
          <div>
            <div className="card-title">Equity curves</div>
            <div className="card-sub">union timeline with forward-filled gaps</div>
          </div>
          <span className="chip">{allTs.length.toLocaleString()} timestamps</span>
        </div>
        <div
        ...
        >
          <Charts.LineChart series={chartSeries.length ? chartSeries : [{ values: [0], color: "var(--accent)" }]} height={280} />
          {hoverIdx != null && allTs[hoverIdx] && (
            <div style={{ position: "absolute", top: 8, left: 16, background: "var(--surface-2)",
                          border: "1px solid var(--border-strong)", borderRadius: 6,
                          padding: "8px 12px", fontSize: 12, zIndex: 10 }}>
              <div className="mono" style={{ marginBottom: 6 }}>{allTs[hoverIdx]?.slice(0, 10)}</div>
        ...
        <div className="row wrap" style={{ gap: 16, marginTop: 14 }}>
          {selectedIds.map((id, i) => (
            <div key={id} className="row" style={{ gap: 8, alignItems: "center" }}>
              ...
              <div className="mono" style={{ fontSize: 12 }}>{id}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-h">
          <div>
            ...
            <div className="card-sub">one column per selected run</div>
          </div>
        </div>
        <div className="tbl-wrap">
          <table className="tbl">
            <thead>
              <tr>
                <th>Metric</th>
                {selectedIds.map((id) => <th key={id} className="num">{id.slice(-8)}</th>)}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={key}>
                  ...
                  {selectedIds.map((id) => {
                    ...
                    return <td key={id} className="num">{kind === "pct" ? fmt.pct(v) : fmt.num(v, 2)}</td>;
                  })}
                </tr>
              ))}
            </tbody>
      ...
      <div className="card">
        <div className="card-h">
          <div>
            ...
            <div className="card-sub">config/risk.yaml</div>
          </div>
          <span className="chip profit">Config loaded</span>
        </div>
      ...
      <div className="row" style={{ gap: "var(--gap-lg)" }}>
        <div className="card" style={{ flex: 1 }}>
          ...
          <table className="tbl">
            ...
            <tbody>
              <tr><td>WS reconnects</td><td className="num">0</td><td className="num">{cfg.ws_reconnect_circuit_threshold ?? 3}</td><td><span className="chip profit">OK</span></td></tr>
              <tr><td>REST error rate</td><td className="num">N/A</td><td className="num">{fmt.pct(cfg.rest_error_rate_circuit_threshold ?? 0.05)}</td><td><span className="chip">demo</span></td></tr>
              <tr><td>Stale quote deviation</td><td className="num">N/A</td><td className="num">{fmt.pct(cfg.stale_quote_pct ?? 0.02)}</td><td><span className="chip">demo</span></td></tr>
              <tr><td>Hard stop cooldown</td><td className="num">inactive</td><td className="num">{cfg.hard_stop_cooldown_hours ?? 48}h</td><td><span className="chip">inactive</span></td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="card" style={{ flex: 1 }}>
          ...
          <button className="btn primary" disabled style={{ width: "100%", opacity: 0.5 }}>Promote to LIVE</button>
        </div>
      </div>
    </div>
  );
